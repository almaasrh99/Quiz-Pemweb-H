/*!
 * jQuery fancyMessenger plugin v1.0.4
 * https://github.com/myspace-nu
 *
 * Copyright 2021 Johan Johansson
 * Released under the MIT license
 */

!(function (e) {
  var s = [];
  (e.fn.fancyMessenger = function (n) {
    s.push(this);
    var t = e.extend(
        !0,
        {
          text: {
            heading: "Kirm pesan pada kami!",
            body: '<form><input class="form-control form-control-sm" type="email" placeholder="Alamat e-mail kamu" /><textarea class="form-control form-control-sm" placeholder="Kirim pesan disini"></textarea><button class="btn btn-light btn-sm">Kirim</button></form>',
            sent: "Message sent!",
            invalidEmail: "e-mail tidak valid!",
            sendError: "Gagal mengirim pesan!",
          },
          available: {
            timezone: "Asia/Jakarta",
            fromHour: 0,
            untilHour: 0,
          },
          closeOnSend: !0,
          onSend: function () {
            return !0;
          },
        },
        n
      ),
      i = function (e, s) {
        const n = e.toLocaleString("ja", { timeZone: s }).split(/[/\s:]/);
        n[1]--;
        const t = Date.UTC.apply(null, n);
        return (new Date(e).setMilliseconds(0) - t) / 60 / 1e3;
      },
      M = this.length ? this : e("body").first(),
      o = e(
        '<div class="fancyMessenger"><div class="fancyMessenger-header"><div class="fancyMessenger-avatar"><div class="fancyMessenger-available">&check;</div></div><div class="fancyMessenger-text">' +
          t.text.heading +
          '</div></div><div class="fancyMessenger-body">' +
          t.text.body +
          '</div><div class="fancyMessenger-info"></div></div>'
      ),
      a = e(
        '<style type="text/css">\n.fancyMessenger{background-color:#2368af;border-radius:28px;max-width:250px;min-width:56px;display:inline-block;box-shadow:0 0 20px rgba(60,60,60,.2)}.fancyMessenger *{font-family:"Segoe UI",Roboto,"Helvetica Neue",Arial,"Noto Sans","Liberation Sans",sans-serif,"Apple Color Emoji","Segoe UI Emoji","Segoe UI Symbol","Noto Color Emoji"}.fancyMessenger-header{position:relative;height:56px;cursor:pointer}.fancyMessenger-body,.fancyMessenger-info{display:none;color:#fff;margin:5px 5px 15px 5px;font-family:Roboto,sans-serif;font-size:12px}.fancyMessenger-body *{margin-bottom:5px}.fancyMessenger-avatar{background-image:url(data:image/svg+xml;base64,);background-position:center;background-size:80%;background-repeat:no-repeat;top:5px;left:5px;width:46px;height:46px;border-radius:23px;position:absolute}.fancyMessenger-available{position:absolute;text-align:center;width:18px;height:18px;background-color:#40b681;color:#fff;font-size:12px;font-weight:700;top:0;left:35px;border-radius:100%}.fancyMessenger-text{padding:20px 40px 0 60px;color:#fff;font-family:Roboto,sans-serif;font-size:13px;display:none}.fancyMessenger-info{display:none}.fancyMessenger input,textarea{box-sizing:border-box;display:block;width:100%;color:#495057;border:1px solid #ced4da;padding:.25rem .5rem;font-size:.875rem;border-radius:.2rem}.fancyMessenger textarea{overflow:auto;resize:vertical}.fancyMessenger button:focus,input:focus,textarea:focus{outline:0}.fancyMessenger button,input[type=button]{padding:.25rem .5rem;font-size:.875rem;border-radius:.2rem;color:#212529;background-color:#f8f9fa;border-color:#f8f9fa;display:inline-block;font-weight:400;text-align:center;vertical-align:middle;border:1px solid transparent;cursor:pointer}.fancyMessenger button:active,input[type=button]:active{box-shadow:0 0 0 .25rem rgba(49,132,253,.5)}\n</style>'
      );
    return (
      this.length ||
        e(o).css({ position: "fixed", bottom: "20px", right: "20px" }),
      (new Date().getUTCHours() <
        t.available.fromHour + i(new Date(), t.available.timezone) / 60 ||
        new Date().getUTCHours() >=
          t.available.untilHour + i(new Date(), t.available.timezone) / 60) &&
        e(o).find(".fancyMessenger-available").hide(),
      M.each(function () {
        var s = e(this),
          n = o.appendTo(s);
        e(n)
          .first()
          .find("button")
          .click(function (s) {
            s.preventDefault();
            var i = 0;
            return (
              e(n)
                .find("input[type=email]")
                .each(function () {
                  (e(this).val() && e(this)[0].validity.valid) ||
                    (e(this)
                      .closest(".fancyMessenger")
                      .find(".fancyMessenger-info")
                      .html(t.text.invalidEmail)
                      .fadeIn(600)
                      .delay(3e3)
                      .fadeOut(600),
                    i++);
                }),
              !i &&
                (t.onSend(n)
                  ? (t.closeOnSend &&
                      (e(this)
                        .closest(".fancyMessenger")
                        .find(".fancyMessenger-body")
                        .hide(),
                      e(this)
                        .closest(".fancyMessenger")
                        .find(".fancyMessenger-text")
                        .hide()),
                    t.collapseOnSend &&
                      e(this)
                        .closest(".fancyMessenger")
                        .find(".fancyMessenger-body")
                        .hide(),
                    void e(this)
                      .closest(".fancyMessenger")
                      .find(".fancyMessenger-info")
                      .html(t.text.sent)
                      .fadeIn(600)
                      .delay(3e3)
                      .fadeOut(600))
                  : (e(this)
                      .closest(".fancyMessenger")
                      .find(".fancyMessenger-info")
                      .html(t.text.sendError)
                      .fadeIn(600)
                      .delay(3e3)
                      .fadeOut(600),
                    !1))
            );
          }),
          e(n)
            .find(".fancyMessenger-header")
            .click(function (s) {
              s.stopPropagation(),
                e(this)
                  .closest(".fancyMessenger")
                  .find(".fancyMessenger-body")
                  .toggle();
            }),
          e(n)
            .find(".fancyMessenger-avatar")
            .click(function (s) {
              s.stopPropagation(),
                e(this)
                  .closest(".fancyMessenger")
                  .find(".fancyMessenger-text")
                  .toggle(),
                e(this)
                  .closest(".fancyMessenger")
                  .find(".fancyMessenger-text")
                  .is(":hidden")
                  ? e(this)
                      .closest(".fancyMessenger")
                      .find(".fancyMessenger-body")
                      .hide()
                  : e(this)
                      .closest(".fancyMessenger")
                      .find(".fancyMessenger-body")
                      .show(),
                e(this)
                  .closest(".fancyMessenger")
                  .find(".fancyMessenger-info")
                  .hide();
            });
      }),
      "none" != e(".fancyMessenger-info").css("display") &&
        a.appendTo(e("body").first()),
      this
    );
  }),
    (e.fancyMessenger = function (s) {
      e.fn.fancyMessenger(s);
    });
})(jQuery);
